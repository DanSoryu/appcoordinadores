  ...
  <div v-if="show" class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md relative border border-gray-200">
      ...
      <div class="mb-6">
        ...
        <div v-if="vehiculo" class="bg-gray-50 p-4 rounded-lg border-l-4 border-red-500">
          ...
          <div class="flex">
            ...
            <div class="ml-3">
              <p class="text-sm text-yellow-800">
                <strong>Advertencia:</strong> Esta acción eliminará permanentemente todos los datos asociados a este vehículo, incluyendo historial de mantenimientos y reparaciones.
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  emits: ['close', 'vehiculo-eliminado'],
  setup(props, { emit }) {
    ...
    const handleDelete = async () => {
      ...
      try {
        await executeSubmit(async () => {
          ...
          // Emitir evento con el ID del vehículo eliminado
          emit('vehiculo-eliminado', props.vehiculo.id);
        });
      } catch (error) {
