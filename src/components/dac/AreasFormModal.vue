<template>
	<div v-if="show" class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
		<div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg relative border border-gray-200 overflow-y-auto max-h-[90vh]">
			...
			<h2 class="text-2xl font-bold mb-6 text-blue-600 text-center">Nueva Área</h2>
			<form @submit.prevent="handleSubmit">
				<!-- SECCIÓN ÚNICA: DATOS DE ÁREA -->
				<div class="mb-8 p-6 bg-gradient-to-r from-primary-50 to-blue-50 rounded-xl border-l-4 border-primary-500">
					<div class="flex items-center mb-4">
						<div class="w-8 h-8 bg-primary-500 text-white rounded-full flex items-center justify-center mr-3">
							<font-awesome-icon icon="layer-group" class="w-4 h-4" />
						</div>
						<h3 class="text-xl font-bold text-primary-700">Información del Área</h3>
					</div>
					<div class="space-y-4">
						<div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
							<label class="block mb-2 font-semibold text-gray-700">División *</label>
							<select
								v-model="formData.division_id"
								:class="[
									'input mb-2 w-full',
									formData.division_id ? (divisionIdValid ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50') : 'border-gray-300'
								]"
								required
							>
							...
							<div v-if="formData.division_id && !divisionIdValid" class="text-red-500 text-xs mt-1">
								Debe seleccionar una división
							</div>
							<div class="mt-2">
								<button type="button" @click="abrirDivisionModal" class="text-blue-600 hover:underline focus:outline-none">
									¿No existe la división? Crea una aquí
								</button>
							</div>
						</div>
						
						<div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
							<label class="block mb-2 font-semibold text-gray-700">Nombre del Área *</label>
							<input 
	...
	<DivisionesFormModal 
		...
		@close="showDivisionModal = false"
		@division-guardada="handleDivisionGuardada"
	/>
</template>

<script>
import BaseButton from '../global/BaseButton.vue'
import DivisionesFormModal from './DivisionesFormModal.vue'
...
export default {
	...
	computed: {
		...
		divisionIdValid() {
			return this.formData.division_id && this.formData.division_id !== '';
		},
		
		isFormValid() {
			return this.nombreValid && this.divisionIdValid;
		},
		
		finalFormData() {
			return {
				nombre: this.formData.nombre.trim().toUpperCase(),
				division_id: parseInt(this.formData.division_id)
			};
		}
	},
	created() {
	...
	watch: {
		show(value) {
			if (value) {
				// Cargar divisiones si no están cargadas
	...
	methods: {
		async loadDivisiones() {
			try {
				...
				const response = await apiClient.get('/divisiones');
				console.log('Respuesta de divisiones:', response.data);
				...
				this.toastStore.addToast({
					message: 'Error al cargar la lista de divisiones',
					type: 'error',
					duration: 5000
				});
			}
		...
		async handleSubmit() {
			...
			try {
				await this.executeSubmit(async () => {
					...
					const response = await apiClient.post('/areas', this.finalFormData);
					console.log('Área creada:', response.data);
					
					// Emitir evento con la nueva área
					this.$emit('area-guardada', response.data.area);
				...
				if (error.response?.data?.error) {
					...
					this.toastStore.addToast({
						message: 'Error al crear el área. Por favor, intente nuevamente.',
						type: 'error',
						duration: 5000
					});
				}
		...
		async handleDivisionGuardada(nuevaDivision) {
			// Recargar la lista de divisiones para incluir la nueva
			await this.loadDivisiones();
			// Seleccionar automáticamente la nueva división
			this.formData.division_id = nuevaDivision.id;
		}
	}
};
</script>
